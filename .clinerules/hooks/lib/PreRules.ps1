...
$script:RuleEngine = @(
    ...
    @{
        ...
        ValidationLogic = {
            ...
            # ========== 第3层：值模式检测 ==========
            # 只有值符合真实凭据模式才拦截
            # 已知凭据前缀模式（使用单引号避免变量插值破坏正则字符类）
            $realCredPatterns = @(
                "(?i)=\s*(?:[$dq$sq])sk-[a-zA-Z0-9]{20,}(?:[$dq$sq])",                         # OpenAI/Stripe key
                "(?i)=\s*(?:[$dq$sq])ghp_[a-zA-Z0-9]{36}(?:[$dq$sq])",                              # GitHub token
                "(?i)=\s*(?:[$dq$sq])AKIA[0-9A-Z]{16}(?:[$dq$sq])",                                   # AWS key
                "(?i)=\s*(?:[$dq$sq])xox[baprs]-[a-zA-Z0-9\\-]+(?:[$dq$sq])",                               # Slack token
                # 高熵值凭据（随机字符多）
                "(?i)=\s*(?:[$dq$sq])[a-zA-Z0-9+/]{40,}={0,2}(?:[$dq$sq])"                                 # Base64 长串
            )
            $hasRealCred = $false
